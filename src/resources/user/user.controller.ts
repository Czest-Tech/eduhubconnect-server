...
class UserController implements Controller {
    ...
    private studentUpdate = '/update/student/:id';
    private studentSetttings = '/student/settings/:id';
    ...
    private initialiseRoutes(): void {
        ...
        this.router.put(`${this.path + this.studentUpdate}`, [this.upload?.single('images')], this.updateStudentInfo);
        this.router.get(`${this.path + this.studentSetttings}`, this.getStudentInfo);
    }
    ...
    private updateStudentInfo = async (req: Request, res: Response, next: NextFunction): Promise<Response | void> => {
        try {
            const { id } = req.params;
            const file = req.file as any;
            if (file) {
            ...
            const user = await this.userService.updateStudent({ _id: new mongoose.Types.ObjectId(id) }, req.body) as any
            res.status(201).json(user)
    ...
    private getStudentInfo = async (req: Request, res: Response, next: NextFunction): Promise<Response | void> => {
        try {
            const { id } = req.params;
            const settings = await this.userService.getStudentInfo({ _id: new mongoose.Types.ObjectId(id) });
            res.status(201).json(settings[0])
